...
df_smoke = df_smoke.drop(["ID", "gender", "eyesight(left)", "eyesight(right)", "hearing(left)", "hearing(right)", "AST", "ALT", "Gtp", "oral", "tartar"], axis=1)
...
print()
print('non presenza fumo:',df_smoke.smoking.value_counts()[0], '(% {:.2f})'.format(df_smoke.smoking.value_counts()[0] /df_smoke.smoking.count() * 100))
print('presenza fumo:',df_smoke.smoking.value_counts()[1], '(% {:.2f})'.format(df_smoke.smoking.value_counts()[1] /df_smoke.smoking.count() * 100), '\n')

# Visualizzazione del grafico
#df_smoke['smoking'].value_counts().plot(kind='bar').set_title('smoking')
plt.show()
